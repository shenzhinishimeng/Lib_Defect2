...
public class InspectionStatisticsTypeHomeAdapter extends BaseQuickAdapter<QueryDefect2StatisticsTypePieChartListsResult.ResultBean, BaseViewHolder> {

    public InspectionStatisticsTypeHomeAdapter(int layoutResId, @Nullable List<QueryDefect2StatisticsTypePieChartListsResult.ResultBean> data) {
        super(layoutResId, data);
    }

    @SuppressLint({"WrongConstant", "ClickableViewAccessibility"})
    @Override
    protected void convert(final BaseViewHolder helper, final QueryDefect2StatisticsTypePieChartListsResult.ResultBean item) {
        helper.setText(R.id.tv_count, item.getCnt() + "")
                .setText(R.id.tv_name, item.getCategory())
